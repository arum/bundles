...
public class ASRecordSet extends ASObject {
	...
	public void populate(String[] columnNames, List<List<Object>> rows) {
		...
		List<List<Object>> initialData = rows
				.subList(0, (initialRowCount > rows.size() ? rows.size()
						: initialRowCount)); // NOTE: sublist semantics are
		// [fromIndex, toIndex]
		setInitialData(initialData);
	...
	/**
	 * @param list
	 *            List of JavaBeans, all beans should be of the same type
